$(function() {
	$('#btnUpdate').click(function() {// 保存按钮点击事件
		...
		var data = { "usersid" : $("#usersid").val(),"username" : $("#username").val(),"password" : $("#password").val(),"realname" : $("#realname").val(),"sex" : $("#sex").val(),"birthday" : $("#birthday").val(),"contact" : $("#contact").val(),"regdate" : $("#regdate").val()};
		var loc = $('#basepath').val();// 获取项目根目录
		var url = loc + "users/xupdateUsers.action";// 获取更新数据Controller地址
